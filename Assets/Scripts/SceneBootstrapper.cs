...
public static class SceneBootstrapper
{
    ...
    static readonly string[] ImportedZombiePrefabCandidatePaths =
    {
    ...
    static bool warnedMissingImportedZombiePrefab;
    ...
    {
        return GameConfig.EnableSceneBootstrap && SceneManager.GetActiveScene().name == GameConfig.GameplaySceneName;
    }
    ...
    public static void ExecuteGameplayBuild()
    {
        ...
        gameManager.autoSpawnWaves = true;
    ...
    static GameObject CreatePlayer()
    {
        ...
        // Remove legacy primitive FPS weapon visuals from older bootstraps.
        Transform legacyFpsWeapon = camera.transform.Find("FPS Weapon");
        if (legacyFpsWeapon != null)
        {
            DestroySafe(legacyFpsWeapon.gameObject);
        }

        // Weapon visuals are handled by Shooting.EnsureWeaponModel()
        // to allow asset-based first-person weapon models.
    ...
    {
        // O template serve apenas como referência para Instantiate no GameManager.
        // Fica inativo e fora do mapa — o GameManager instancia clones dele.
        // Os componentes de jogo (ZombieAI, ZombieHealth, etc.) são adicionados
        // pelo GameManager em TrySpawnWaveZombie, não aqui.
        GameObject zombie = TryInstantiateImportedZombiePrefab("Zombie Template", new Vector3(9999f, -999f, 9999f));
        if (zombie == null)
        {
            Debug.LogError("[Bootstrap] CreateZombieTemplate: prefab URP não encontrado. Waves desativadas.");
            GameObject stub = new GameObject("Zombie Template (MISSING PREFAB)");
        ...
        // Componentes mínimos no template para que Instantiate produza clones funcionais
        if (zombie.GetComponent<CapsuleCollider>() == null)
        {
        ...
        {
            var a = zombie.AddComponent<AudioSource>(); a.playOnAwake = false;
        }
        if (zombie.GetComponent<ZombieAI>() == null)    zombie.AddComponent<ZombieAI>();
        if (zombie.GetComponent<ZombieHealth>() == null) zombie.AddComponent<ZombieHealth>();
    ...
    static GameManager CreateGameManager(GameObject zombieTemplate, List<Transform> spawnPoints)
    {
        ...
        // Usa SetZombiePrefabAndStartWaves para garantir que a wave loop arranca
        // mesmo que o GameManager.Start() já tenha corrido antes deste ponto.
        gameManager.SetZombiePrefabAndStartWaves(zombieTemplate);
        return gameManager;
    }
    ...
    {
        GameObject zombie = CreateZombieBase($"{zoneName} Zombie", new Vector3(position.x, 1.05f, position.z), true);
        if (zombie == null) return; // PATCH: prefab ausente — ignora silenciosamente (erro já logado)
        zombie.transform.SetParent(parent);
        ZombieAI ai = EnsureZombieRuntimeComponents(zombie);
        if (ai == null) return;
        ai.variant = variant;
        ai.moveSpeed = 1.55f;
    ...
    static GameObject CreateZombieBase(string zombieName, Vector3 spawnPosition, bool activeState)
    {
        // PATCH: prefab real obrigatório — sem fallback primitivo.
        GameObject importedRoot = TryInstantiateImportedZombiePrefab(zombieName, spawnPosition);
        if (importedRoot != null)
        {
            importedRoot.SetActive(activeState);
            return importedRoot;
        }

        // Prefab não encontrado: loga erro crítico e retorna null para que o caller saiba.
        Debug.LogError(
            "[Bootstrap] ERRO CRÍTICO: prefab zombie URP não encontrado. " +
            "Verifica que 'Assets/ZombieMale_AAB/Prefabs/URP/ZombieMale_AAB_URP.prefab' existe no projecto. " +
            "Nenhum zombie primitivo será criado.");
        return null;
    }

    static ZombieAI EnsureZombieRuntimeComponents(GameObject zombie)
    {
        if (zombie == null)
        {
            return null;
        ...
        CapsuleCollider collider = zombie.GetComponent<CapsuleCollider>();
        if (collider == null)
        {
    ...
    static GameObject TryInstantiateImportedZombiePrefab(string zombieName, Vector3 spawnPosition)
    {
#if UNITY_EDITOR
        GameObject prefab = FindImportedZombiePrefabInEditor();
        if (prefab == null)
        {
            if (!warnedMissingImportedZombiePrefab)
            {
                warnedMissingImportedZombiePrefab = true;
                Debug.LogError("[Bootstrap] Prefab zombie importado NÃO encontrado no Editor. " +
                               "Confirma que ZombieMale_AAB_URP.prefab existe em Assets/ZombieMale_AAB/Prefabs/URP/");
            }
            return null;
        }
        ...
        if (instance == null)
        {
            Debug.LogError("[Bootstrap] PrefabUtility.InstantiatePrefab retornou null para o prefab zombie.");
            return null;
        }

        instance.name = zombieName;
        instance.transform.position = spawnPosition;
        instance.transform.localScale = Vector3.one;
        return instance;
#else
        // Em builds de runtime, carrega o prefab de Resources.
        // Requer que ZombieMale_AAB_URP esteja em Assets/Resources/Prefabs/ZombieMale_AAB_URP.prefab
        GameObject prefab = Resources.Load<GameObject>("Prefabs/ZombieMale_AAB_URP");
        if (prefab == null)
        {
            Debug.LogError("[Bootstrap] RUNTIME: Resources.Load falhou para 'Prefabs/ZombieMale_AAB_URP'. " +
                           "Copia o prefab para Assets/Resources/Prefabs/");
            return null;
        }

        GameObject inst = Object.Instantiate(prefab, spawnPosition, Quaternion.identity);
        inst.name = zombieName;
        return inst;
#endif
    }

#if UNITY_EDITOR
    static GameObject FindImportedZombiePrefabInEditor()
    {
        for (int i = 0; i < ImportedZombiePrefabCandidatePaths.Length; i++)
        {
            GameObject candidate = AssetDatabase.LoadAssetAtPath<GameObject>(ImportedZombiePrefabCandidatePaths[i]);
            if (candidate != null)
            {
                return candidate;
            }
